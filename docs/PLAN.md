...
### 13.9 เอฟเฟกต์สายพันธุ์ — ตัวเลขขึ้นกับ "แรง" ของมอนแต่ละตัว ✅ ตอบแล้ว

มอนสเตอร์ลิงมี "เอฟเฟกต์สายพันธุ์" ผูกกับสายพันธุ์ (ต่างจาก "ลักษณะเฉพาะ" ที่สุ่ม
รายตัว จึงไม่เก็บ) แต่ตัวเลขที่อ่านได้ไม่กลม เช่น 5.78% / 8.67% / 11.55% / 6.57%

**คำตอบ (ยืนยันจากผู้เล่น 2026-09-12):** ตัวเลขขึ้นกับ **"แรง" ของมอนตัวนั้น ๆ**
ไม่ใช่ค่าคงที่ของสายพันธุ์ และไม่ใช่การสเกลตามเลเวล แรงมี 5 ขั้น:

> เทา → เขียว → ฟ้า → ม่วง → **ทอง (สูงสุด)**

**กติกาของโปรเจกต์: เก็บเฉพาะค่าจากมอนสีทอง** เพราะคนมาดูเว็บเพื่อจะบิ้ว
ไม่มีใครเอาตัวแรงต่ำมาใช้ ค่าที่แสดงจึงเป็น "เพดานของสายพันธุ์นี้"

ผลที่ตามมาในโครงข้อมูล:

- `monsterling.effectsReadFrom` เป็นฟิลด์บังคับเมื่อมี `speciesEffects`
  เก็บทั้ง `level` และ `grade` (คำศัพท์กลุ่ม `monsterGrade`)
- `validate:data` เตือนทุกไฟล์ที่ `grade` ไม่ใช่ `gold` ว่า **ยังใช้แสดงบนเว็บไม่ได้**
- หน้าเว็บต้องกำกับว่าเป็น "ค่าสูงสุด (แรงสีทอง)" ไม่ใช่ค่าที่ทุกตัวจะได้
  ไม่งั้นคนจับมอนสีเขียวมาแล้วจะงงว่าทำไมเลขไม่ตรง

**ของที่ต้องแคปใหม่:** ก็อบลิน ร้อยตรี อ่านมาจากตัวเลเวล 1 ที่ยังไม่รู้แรง (6.57%)
ติดธงไว้แล้วทั้งใน `fieldsUnverified` และคำเตือนของ validator
