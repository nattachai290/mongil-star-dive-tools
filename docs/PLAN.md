...
### 13.9 เอฟเฟกต์สายพันธุ์ — ตัวเลขสเกลตามเลเวลหรือเปล่า ⚠️ ยังไม่รู้

มอนสเตอร์ลิงแต่ละตัวมี "เอฟเฟกต์สายพันธุ์" ผูกกับสายพันธุ์ (ต่างจาก "ลักษณะเฉพาะ"
ที่สุ่มรายตัว จึงไม่เก็บ — ดู §13 เดิม) แต่ตัวเลขที่อ่านได้ชวนสงสัย:

| มอน | ค่า | เลเวล |
|---|---|---|
| ผู้พิทักษ์แห่งนครทองคำ | **5.78%** | 60 |
| ปีศาจเสือบิดเบี้ยว | **8.67%** | 60 |
| ต้นสนผู้เฒ่าอี | 8% | 60 |
| ฮันอุล | 11% | 60 |
| กายหมัดน้ำแข็ง | 11% | 60 |

5.78 กับ 8.67 ไม่ใช่เลขกลม — เลขแบบนี้มักเป็นค่าที่**คำนวณมาจากเลเวลหรือดาว**
ไม่ใช่ค่าคงที่ของสายพันธุ์ ถ้าจริงแล้วไปกรอกทั้ง 169 ตัวโดยไม่บันทึกเลเวล
ข้อมูลจะใช้ไม่ได้ทันทีที่มีคนเปิดดูมอนเลเวลอื่น และย้อนแก้ไม่ได้ด้วย
เพราะไม่มีใครจำได้ว่าตอนแคปมอนอยู่เลเวลไหน

**ทางออกที่ใช้ไปก่อน:** `monsterling.effectsAtLevel` เป็นฟิลด์บังคับเมื่อมี
`speciesEffects` (schema บังคับเอง มี selftest คุม) ตอนนี้ทุกตัวที่กรอกแล้วเป็น
เลเวล 60 สูงสุด ข้อมูลจึงถูกต้องเสมอไม่ว่าคำตอบจะเป็นอะไร

**หลักฐานรอบที่สอง (2026-09-12):** ได้ข้อมูลมอนเลเวล 1 มาแล้วหนึ่งตัว

| มอน | เลเวล | เอฟเฟกต์สายพันธุ์ | ลักษณะเฉพาะ (ไม่เก็บ) |
|---|---|---|---|
| ก็อบลิน ร้อยตรี | **1** | **6.57%** | 1.5% / 7.5% / 2% / 6% |
| ตัวอื่นในชุดเดียวกัน | 60 | 5.78–11.55% | 10% / 15% / 15% / 10% |

อ่านได้สองอย่าง:

1. **ลักษณะเฉพาะสเกลแน่นอน** — ตัวเลเวล 1 ได้ 1.5–7.5% ส่วนเลเวล 60 ได้ 10–15%
   (ยิ่งเป็นเหตุผลที่ไม่เก็บ นอกเหนือจากที่มันสุ่มรายตัวอยู่แล้ว)
2. **เอฟเฟกต์สายพันธุ์อาจไม่สเกล** — 6.57% ของตัวเลเวล 1 อยู่ในช่วงเดียวกับ
   ตัวเลเวล 60 (5.78–11.55%) ไม่ได้ต่ำกว่าอย่างมีนัย

ถ้าไม่สเกลจริง เลขทศนิยมก็ต้องมาจากที่อื่น — เดาว่าสุ่มรายตัวตอนจับเหมือนลักษณะเฉพาะ
ซึ่งถ้าจริง **ห้ามเก็บตัวเลขเป็นข้อมูลของสายพันธุ์เลย** ต้องเก็บแค่รูปแบบเอฟเฟกต์

**การทดสอบที่ชี้ขาด:** หามอน**สายพันธุ์เดียวกันสองตัว**ในกระเป๋า (มี 2,942 ตัว
น่าจะซ้ำเยอะ) แล้วเทียบ % ของเอฟเฟกต์สายพันธุ์

| ผลลัพธ์ | แปลว่า | ต้องทำ |
|---|---|---|
| เลขเท่ากันทั้งคู่ | เป็นค่าของสายพันธุ์จริง | เก็บตัวเลขต่อไปได้ |
| เลขต่างกัน | สุ่มรายตัว | ลบ `value` ทิ้ง เก็บแค่ kind/stat/trigger |

จนกว่าจะรู้ `effectsAtLevel` ทำหน้าที่กันความเสียหายไว้
