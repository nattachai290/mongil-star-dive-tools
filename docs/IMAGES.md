...
> **หนึ่งภาพได้มอน 8 ตัวเท่านั้น** — ต้องแคปหลายภาพโดย**เลื่อนหน้าจอไปคนละตำแหน่ง**
> คัดลอกภาพเดิมหลายไฟล์ไม่ได้ทำให้ได้มอนเพิ่ม (ได้มอน 8 ตัวเดิม แต่ถูกเซฟเป็นชื่อของมอนตัวอื่น)
> สคริปต์จะหยุดทันทีถ้าเจอภาพที่เนื้อในเหมือนกันเป๊ะ

### 2. วางไว้ใน `assets-src/codex/` และตั้งชื่อตาม **เลขบนการ์ดซ้ายบนสุด**
...
> ⚠️ **ไม่ใช่ลำดับภาพ** — ภาพที่สองมักชื่อ `9.png` ไม่ใช่ `2.png`
> อ่านเลขจากการ์ดซ้ายบนสุดในภาพนั้นแล้วใช้เป็นชื่อไฟล์ ไม่ต้องคำนวณ

```
assets-src/codex/
  1.png      <- การ์ดซ้ายบนเขียน No.1   (ได้ No.1–8)
  9.png      <- การ์ดซ้ายบนเขียน No.9   (ได้ No.9–16)
  17.png     <- การ์ดซ้ายบนเขียน No.17  (ได้ No.17–24)
  ...
```

ถ้าตั้งชื่อผิดเป็น 1,2,3… ช่วงจะซ้อนกันแล้วได้ไฟล์ไม่ครบ — สคริปต์จะเตือนและบอกว่าขาดตัวไหนบ้าง

ชื่อต้องเป็นตัวเลขล้วน นามสกุล `.png` `.jpg` หรือ `.jpeg`

**สมุดเล่มอื่นใส่ชื่อเล่มนำหน้า** เพราะแต่ละเล่มเริ่มนับ No.1 ใหม่ เลขอย่างเดียวชี้ตัวไม่ได้

```
event-1.png       <- แท็บ Events (3 ตัว)
legendary-1.png   <- แท็บ Legendary Monsters (ตอนนี้มีชื่อแค่ No.1)
```

ภาพที่การ์ดไม่เต็ม 8 ใบไม่เป็นไร ช่องที่เลยท้ายเล่มจะถูกข้ามพร้อมบอกเหตุผล
...
ท้ายผลจะสรุปว่า **ได้ไฟล์จริงกี่รูป** และ **ยังขาดมอนตัวไหน** — ดูตัวเลขนี้ ไม่ใช่จำนวนครั้งที่ตัด
